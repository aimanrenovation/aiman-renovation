      ...
      <div className="min-h-screen bg-[#0A0A0A] flex items-center justify-center p-4">
        <div className="bg-[#111111] rounded-3xl p-10 w-full max-w-md text-center border border-white/10">
          ...
          <div className="space-y-3">
            <Link
              href="/"
              className="flex items-center justify-center w-full h-9 bg-[#E50000] hover:bg-red-700 text-white rounded-lg text-sm font-medium"
            >
              <ArrowLeft className="w-4 h-4 mr-2" /> Retour a l&apos;accueil
            </Link>
            <a
              href="tel:0633496925"
              className="flex items-center justify-center w-full h-9 border border-white/30 text-white hover:bg-white/10 rounded-lg text-sm font-medium"
            >
                ...
                {ZONES_CONFIG.map((zone) => {
                  ...
                  return (
                    <button
                      key={zone.id}
                      onClick={() =>
                        dispatch({ type: "TOGGLE_ZONE", zone: zone.id })
                      }
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`font-medium ${isSelected ? "text-[#E50000]" : "text-white"}`}
                        >
                          {zone.labelKey}
                        </span>
                      </div>
                  ...
                  <div key={zoneId} className="space-y-2">
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {zoneConfig.problems.map((problem) => {
                        const isSelected = state.problems[zoneId].includes(
                          problem.id,
                        );
                        return (
                          <button
                  ...
                  <div key={zoneId} className="space-y-2">
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {zoneConfig.renovationOptions.map((option) => {
                        const isSelected = state.renovationOptions[
                          zoneId
                        ].includes(option.id);
                        return (
                          <button
                          ...
                          >
                            ...
                            <div>
                              <p
                                className={`text-sm font-medium ${isSelected ? "text-[#E50000]" : "text-white"}`}
                              >
                                {option.label}
                              </p>
                ...
                <div className="grid grid-cols-1 gap-2">
                  {BUDGET_OPTIONS.map((option) => (
                    <button
                      key={option.value}
                      onClick={() =>
                        dispatch({ type: "SET_BUDGET", budget: option.value })
                      }
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          {state.problems[zoneId].map((pId) => {
                            const p = zoneConfig.problems.find(
                              (prob) => prob.id === pId,
                            );
                            return (
                              <Badge
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          {state.renovationOptions[zoneId].map((oId) => {
                            const o = zoneConfig.renovationOptions.find(
                              (opt) => opt.id === oId,
                            );
                            return (
                              <Badge
          ...
          <div className="flex justify-between mt-8">
            ...
            {state.currentStep < 5 ? (
              <Button
                size="lg"
                disabled={
                  state.currentStep === 0 && state.selectedZones.length === 0
                }
                onClick={() => {
                  if (
                    state.currentStep === 0 &&
